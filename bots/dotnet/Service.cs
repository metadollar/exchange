...
namespace MetaDollar.Bots.DotNet
{
    public class Service
    {
        ...
        public Service(MetaDollarConfiguration config, ILogger configLogger)
        {
            ...
            var addressMetaDollar = Web3.ToChecksumAddress(config.AddressMetaDollar);

            // TODO: check file exists
            var abi = File.ReadAllText(config.AbiFile);
            MetaDollarContract = Web3.Eth.GetContract(abi, addressMetaDollar);
        ...
        public MetaDollarConfiguration Config { get; }
        public Web3 Web3 { get; }
        public Contract MetaDollarContract { get; }
        ...
        internal async Task<TransactionReceipt> TakeOrder(Order order, BigInteger amount)
        {
            ...
            var fnTest = MetaDollarContract.GetFunction("testTrade");
            ...
            var fnTrade = MetaDollarContract.GetFunction("trade");
            ...
            var encoded = Web3.OfflineTransactionSigner.SignTransaction(Config.PrivateKey, Config.AddressMetaDollar, amount,
        ...
        internal Order CreateOrder(OrderType orderType, BigInteger expires, BigInteger price, BigInteger amount)
        {
            ...
            var contractAddr = Config.AddressMetaDollar;
        ...
        internal async Task<BigInteger> GetMetaDollarBalance(string token, string user)
        {
            BigInteger balance = 0;
            ...
            try
            {
                ...
                var tokenFunction = MetaDollarContract.GetFunction("balanceOf");
                balance = await tokenFunction.CallAsync<BigInteger>(token, user);
                Log("METADOLLAR- GET BALANCE");
            }
            catch (Exception ex)
            {
                Log(ex.Message);
