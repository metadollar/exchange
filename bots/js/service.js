...
const ABIMetaDollar = require('../contracts/metadollar.json');
...
function Service() {
  ...
  self.init = config => new Promise((resolve, reject) => {
    ...
    self.contractMetaDollar = self.web3.eth.contract(ABIMetaDollar).at(config.addressMetaDollar);
  ...
  self.getMetaDollarBalance = (token, user) => new Promise((resolve, reject) => {
    self.contractMetaDollar.balanceOf(token === 'ETH' ? '0x0000000000000000000000000000000000000000' : token.addr, user.addr, (err, result) => {
      if (err) reject(err);
      resolve(result);
    });
  });
  ...
  self.takeOrder = (user, order, fraction) => new Promise((resolve, reject) => {
    ...
    .then((nonce) => {
      ...
      self.contractMetaDollar.testTrade.call(
        order.tokenGet,
        order.amountGet,
        order.tokenGive,
        order.amountGive,
        ...
        (errTest, resultTest) => {
          ...
          const data = self.contractMetaDollar.trade.getData(
            order.tokenGet,
            order.amountGet,
            order.tokenGive,
            order.amountGive,
          ...
          const options = {
            ...
            nonce,
            data,
            to: self.config.addressMetaDollar,
          };
  ...
  self.createOrder = (side, expires, price, amount, token, user) => {
    ...
    const contractAddr = self.config.addressMetaDollar;
