#!/usr/bin/python3
#
# metadollarclientservice.py: MetaDollar API Client Service for python3 using websockets
# ===============================================================================
#
...
# MetaDollar contract address
# This rarely changes.
addressMetaDollar = '0x9fB587Fc50B4c5ba8e4947DD9cfbBE7e76A25486'    # metadollar_2's contract address
# Global API interfaces
web3 = Web3(HTTPProvider('https://mainnet.infura.io/nrFgJL37tva2NWGjddYJ'))

class MetaDollarClientService:
    ...
    def getMetaDollarBalance(self, token, user):
        user = Web3.toChecksumAddress(user)
        if token == 'ETH':
            balance = self.contractMetaDollar.call().balanceOf(token="0x0000000000000000000000000000000000000000", user=user)
        else:
            token = Web3.toChecksumAddress(token)
            balance = self.contractMetaDollar.call().balanceOf(token=token, user=user)
    ...
    def printBalances(self, token, userAccount):
        ...
        print("MetaDollar ETH balance: %.18f ETH" % self.getMetaDollarBalance('ETH', userAccount))
        print("MetaDollar token balance: %.18f tokens" % self.getMetaDollarBalance(token, userAccount))

    def createOrder(self, side, expires, price, amount, token, userAccount, user_wallet_private_key, randomseed = None):
        global addressMetaDollar, web3
        ...
        hashhex = self.soliditySha256(
            ...
            [addressMetaDollar, tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce]
        )
        ...
        orderDict = {
            ...
            'tokenGet' : tokenGet,
            'tokenGive' : tokenGive,
            'contractAddr' : addressMetaDollar,
            'expires' : expires,
            'nonce' : orderNonce,
            'user' : userAccount,
            'v' : signresult['v'],
    ...
    def trade(self, order, etherAmount, user_wallet_private_key=''):
        ...
        abidata = self.contractMetaDollar.encodeABI('trade', kwargs=kwargs)
        ...
        transaction = { 'to': addressMetaDollar, 'from': self.userAccount, 'gas': maxGas, 'gasPrice': gasPriceWei, 'data': abidata, 'nonce': nonce, 'chainId': 1}
        print(transaction)
    ...
    def on_message(self, ws, message):
        ...
        if 'market' in j:
            ...
            if 'trades' in market:
                ...
                print("WARNING: no trades found in market response from MetaDollar API, this happens from time to time but we don't really need it here so not retrying.")
            # Fill the list of orders
            if 'orders' in market:
                print("INFO: market reply contains orderbook")
                self.updateOrders(j[1]['orders'])
            else:
                print("WARNING: market response from MetaDollar API did not contain order book, this happens from time to time, retrying after a 10 second grace period...")
                time.sleep(10)
                self.send_getMarket()
        elif 'orders' in j:
            ...
            print("Received funds event from MetaDollar API, no action to take.")
        elif 'messageResult' in j:
            messageResult = j[1]
            print("Received messageresult from MetaDollar API, result is: " + str(messageResult))
        else:
            print("Received an unrecognized event from the MetaDollar API, no action to take.")
    ...
    def on_pong(self, ws, pong):
        print('MetaDollar WebSocket API replied to our ping with a pong:' + str(pong))
    ...
    def on_open(self, ws):
        #print("MetaDollar WebSocket connected")
        if len(self.orders_sells) == 0:
            self.send_getMarket()

    def websocket_connect(self):
        self.ws = websocket.WebSocketApp(
            "wss://socket.metadollar.com/socket.io/?transport=websocket",
    ...
    def start(self, userAccount, token):
        ...
        def run(*args):
            ...
            while True:
                time.sleep(10)
                print("MetaDollarClientService thread running...")
        _thread.start_new_thread(run, ())

    def __init__(self):
        global addressMetaDollar

        addressMetaDollar = Web3.toChecksumAddress(addressMetaDollar)

        # Load the ABI of the MetaDollar contract
        with open('../contracts/metadollar.json', 'r') as abi_definition:
            abiMetaDollar = json.load(abi_definition)
        self.contractMetaDollar = web3.eth.contract(address=addressMetaDollar, abi=abiMetaDollar)
